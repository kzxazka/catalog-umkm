...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'"
    ...
    @if(in_array(auth()->user()->role, ['admin', 'superadmin']))
        <div class="mb-8 px-2 flex items-center gap-3">
            <img src="{{ asset('img/disperdaglogo.png') }}" class="w-10 h-10 object-contain shrink-0" alt="Logo Disperdag">
            <div>
                <h2 class="text-sm font-bold text-primary leading-tight">GALERI UKM BDL</h2>
                <p class="text-[11px] text-on-surface-variant">Dinas Perdagangan</p>
                <p class="text-[11px] text-on-surface-variant">Kota Bandar Lampung</p>
            </div>
        </div>
    @else
        <div class="mb-8 px-2">
    ...
    <nav class="flex-1 space-y-0.5">
        ...
        @if(in_array(auth()->user()->role, ['admin', 'superadmin']))
            ...
            <a href="{{ route('admin.events.index') }}"
                class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all
                      {{ request()->routeIs('admin.events*') ? 'bg-primary text-white' : 'text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface' }}">
            ...
            <a href="{{ route('admin.mitra') }}"
                ...
                @if($mitraCount > 0)
                    <span
                        class="ml-auto px-2 py-0.5 bg-secondary text-white text-[10px] font-bold rounded-full">{{ $mitraCount }}</span>
                @endif
            </a>
        ...
        @else
            ...
            <a href="{{ route('products.index') }}"
                class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all
                      {{ request()->routeIs('products.*') ? 'bg-primary text-white' : 'text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface' }}">
