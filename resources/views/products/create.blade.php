...
<x-app-layout>
    <!-- Header Section -->
    <div class="mb-8">
        <h1 class="font-headline-lg text-3xl font-bold text-primary mb-2">Tambah Produk</h1>
        <p class="font-body-md text-on-surface-variant">
            Tambahkan produk baru ke dalam etalase toko Anda (Kategori Toko: 
            <span class="font-bold text-primary text-sm uppercase tracking-wider bg-primary/10 px-2 py-0.5 rounded">{{ $storeCategory }}</span>).
        </p>
    </div>
                ...
                <div class="bg-surface border border-outline-variant rounded-xl shadow-sm p-6">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Sepatu Sneakers Running Premium"
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="5" required placeholder="Tuliskan spesifikasi, keunggulan, ukuran, bahan, dll secara lengkap..."
                                class="w-full px-4 py-2 bg-surface-container border border-outline-variant rounded-lg text-sm focus:border-primary focus:ring-1 focus:ring-primary transition-all">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-bold text-on-surface mb-2">
                                Unggah File Foto <span class="text-red-500 font-bold">* Wajib min. 3 foto</span>
                            </label>
                            
                            <!-- Custom File Upload Container -->
                            <div class="flex items-center justify-center w-full">
                                <label class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-outline-variant rounded-xl cursor-pointer bg-surface-container-low hover:bg-surface-container transition-colors">
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                        <span class="material-symbols-outlined text-4xl text-primary mb-2">cloud_upload</span>
                                        <p class="mb-1 text-sm text-on-surface font-semibold">Pilih minimal 3 gambar</p>
                                        <p class="text-xs text-on-surface-variant">PNG, JPG, JPEG atau WEBP (Maks. 3MB/foto)</p>
                                    </div>
                                    <input type="file" name="photos[]" required multiple accept="image/*" class="hidden" onchange="previewPhotos(event)">
                                </label>
                            </div>
                            ...
                            <p class="text-xs text-on-surface-variant mt-2 leading-relaxed">
                                Foto produk otomatis di-resize dan dikompresi menjadi format WebP agar performa katalog tetap super cepat dan hemat kuota.
                            </p>
                        </div>
