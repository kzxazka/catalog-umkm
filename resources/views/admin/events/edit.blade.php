<x-app-layout>
    <div class="min-h-screen max-w-3xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="font-headline-lg text-3xl font-bold text-primary">Edit Event</h1>
            <p class="font-body-md text-on-surface-variant mt-1">Perbarui informasi event, pameran, atau panggung UMKM lokal.</p>
        </div>
        ...
        <div class="bg-white border border-outline-variant rounded-2xl shadow-sm overflow-hidden p-6 sm:p-8">
            <form method="POST" action="{{ route('admin.events.update', $event->id) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $event->title) }}" required placeholder="Contoh: Lampung Clothing & Craft Expo 2026"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="6" required placeholder="Tuliskan detail jadwal, pemateri, target pengunjung, dan benefit event secara lengkap..."
                              class="w-full px-4 py-3 bg-surface-container-low border border-outline-variant rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none text-sm resize-none transition-all font-sans">{{ old('description', $event->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" id="location" name="location" value="{{ old('location', $event->location) }}" required placeholder="Contoh: Hall A PKOR Way Halim"
                               class="w-full px-4 py-3 bg-surface-container-low border border-outline-variant rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none text-sm transition-all">
                    </div>
                </div>
                ...
                <!-- Upload Banner Event -->
                <div>
                    <label class="block text-xs font-black uppercase tracking-wider text-on-surface-variant mb-2">Banner / Foto Event</label>
                    
                    <div x-data="{ fileName: '', previewUrl: '{{ $event->image ? asset('storage/events/' . $event->image) : '' }}' }" class="relative border-2 border-dashed border-outline-variant hover:border-primary rounded-2xl p-6 transition-colors flex flex-col items-center justify-center text-center bg-surface-container-low/40">
                        ...
                        <template x-if="previewUrl">
                            <div class="space-y-3 z-20">
                                ...
                                <p class="text-xs font-bold text-primary truncate max-w-xs" x-text="fileName ? fileName : 'Banner Event Saat Ini'"></p>
                                <span class="text-[10px] text-on-surface-variant bg-surface-container px-2 py-0.5 rounded">Ketuk untuk mengganti banner</span>
                            </div>
                        </template>
                ...
                <div>
                    ...
                    <select id="status" name="status" class="w-full px-4 py-3 bg-surface-container-low border border-outline-variant rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none text-sm transition-all cursor-pointer">
                        <option value="active" {{ old('status', $event->status) === 'active' ? 'selected' : '' }}>Aktif (Tampilkan Langsung di Halaman Depan)</option>
                        <option value="draft" {{ old('status', $event->status) === 'draft' ? 'selected' : '' }}>Draft (Simpan, Sembunyikan Dulu)</option>
                    </select>
                </div>
