{{-- ═══════════ HERO ═══════════ --}}
<section
    class="bg-gradient-to-br from-primary via-primary-container to-primary text-white py-8 md:py-14 px-4 relative overflow-hidden">
    <div class="absolute inset-0 opacity-5" style="background-image:url(\" data:image/svg+xml,%3Csvg width='60'
        height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg' %3E%3Cg fill='%23fff' fill-opacity='1'
        %3E%3Cpath
        d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'
        /%3E%3C/g%3E%3C/svg%3E\")"></div>
    <div class="max-w-screen-xl mx-auto relative">
        <div
            class="inline-flex items-center gap-1.5 bg-white/10 border border-white/20 text-white text-[11px] font-bold px-3 py-1 rounded-full mb-3">
            <span class="material-symbols-outlined" style="font-size:13px">verified</span>
            UMKM Terverifikasi Resmi
        </div>
        <h1 class="text-2xl md:text-4xl font-black mb-2 leading-tight">Etalase Digital<br class="sm:hidden" /> UMKM
            Lokal</h1>
        <p class="text-blue-200 text-sm md:text-base max-w-lg">Produk autentik dari pengrajin dan pelaku usaha lokal
            terverifikasi Dinas Perdagangan</p>
    </div>
</section>
...
<div class="max-w-screen-xl mx-auto px-4 md:px-6 py-4 md:py-6">
    ...
    @guest
        <div
            class="bg-gradient-to-r from-primary to-primary-container rounded-2xl p-5 mb-5 flex items-start gap-4 relative overflow-hidden shadow-sm">
            ...
            <div class="flex-1">
                ...
                <p class="text-blue-200 text-xs md:text-sm mb-3 leading-relaxed">Promosikan produk Anda secara resmi di
                    bawah binaan Dinas Perdagangan Kota Bandar Lampung. Gratis & terverifikasi.</p>
                <div class="flex gap-2 flex-wrap">
                    <a href="{{ route('register') }}"
                        class="bg-secondary text-white text-xs font-bold px-4 py-2 rounded-xl hover:opacity-90 transition-opacity">
                        Daftar Mitra Sekarang
                    </a>
                    <a href="{{ route('login') }}"
                        class="border border-white/30 text-white text-xs font-bold px-4 py-2 rounded-xl hover:bg-white/10 transition-colors">
                        Masuk Mitra
                    </a>
                </div>
            </div>
    ...
    @else
        @if(auth()->user()->role === 'buyer')
            @if(auth()->user()->hasPendingMitraApplication())
                <div
                    class="bg-gradient-to-r from-amber-500 to-amber-600 rounded-2xl p-5 mb-5 flex items-start gap-4 relative overflow-hidden shadow-sm">
                    <div class="absolute -right-4 -bottom-4 opacity-10 text-[80px] select-none">⏳</div>
                    <div class="w-10 h-10 bg-white/15 rounded-xl flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div class="flex-1 text-white">
                        ...
                        <p class="text-amber-100 text-xs md:text-sm mb-3 leading-relaxed">Berkas pendaftaran Anda sedang
                            diverifikasi oleh Admin Dinas Perdagangan. Silakan pantau secara berkala.</p>
                        <div class="flex gap-2 flex-wrap">
                            <a href="{{ route('mitra.status') }}"
                                class="bg-white text-amber-700 text-xs font-bold px-4 py-2 rounded-xl hover:bg-amber-50 transition-colors">
            ...
            @else
                <div
                    class="bg-gradient-to-r from-red-500 to-orange-500 rounded-2xl p-5 mb-5 flex items-start gap-4 relative overflow-hidden shadow-sm">
                    <div class="absolute -right-4 -bottom-4 opacity-10 text-[80px] select-none">📋</div>
                    <div class="w-10 h-10 bg-white/15 rounded-xl flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div class="flex-1 text-white">
                        ...
                        <p class="text-red-100 text-xs md:text-sm mb-3 leading-relaxed">Anda belum mengirimkan berkas pendaftaran
                            mitra. Mohon lengkapi formulir pendaftaran agar dapat diverifikasi.</p>
                        <div class="flex gap-2 flex-wrap">
                            <a href="{{ route('mitra.register') }}"
                                class="bg-white text-red-700 text-xs font-bold px-4 py-2 rounded-xl hover:bg-red-50 transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($events as $event)
                    <a href="{{ route('events.show', $event->id) }}"
                        ...
                        {{-- Event Image/Banner --}}
                        <div
                            class="w-full sm:w-40 aspect-[16/9] sm:aspect-square bg-surface-container shrink-0 overflow-hidden relative border-b sm:border-b-0 sm:border-r border-outline-variant/40">
                            @if($event->image)
                        ...
                        <div class="p-4 flex-1 flex flex-col justify-between">
                            <div>
                                <div class="flex items-center gap-2 mb-1.5 flex-wrap">
                                    <span
                                        class="inline-flex items-center gap-0.5 text-[9px] font-black text-primary bg-primary/5 px-2 py-0.5 rounded-full border border-primary/10 uppercase tracking-widest">
                                    ...
                                    <span
                                        class="inline-flex items-center gap-0.5 text-[9px] font-black text-on-surface-variant bg-surface-container px-2 py-0.5 rounded-full uppercase tracking-widest">
                                        <span class="material-symbols-outlined" style="font-size:11px">location_on</span>
                                        {{ Str::limit($event->location, 18) }}
                                    </span>
                                </div>
                                <h3
                                    class="font-bold text-xs sm:text-sm uppercase text-on-surface tracking-tight group-hover:text-primary transition-colors leading-snug line-clamp-2 mb-2">
                                    {{ $event->title }}
                                </h3>
                            ...
                            <div class="flex items-center justify-between mt-3 pt-2.5 border-t border-outline-variant/40">
                                <span
                                    class="text-[9px] font-black text-primary uppercase tracking-wider flex items-center gap-0.5">
                                    Detail Informasi <span class="material-symbols-outlined"
                                        style="font-size:12px">arrow_right_alt</span>
                                </span>
                            </div>
                        </div>
                    </a>
    ...
    @if($products->isNotEmpty())
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4">
            @foreach($products as $product)
                ...
                @endphp
                <div
                    class="relative bg-white rounded-2xl border border-outline-variant overflow-hidden group hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                    ...
                    <a href="{{ route('catalog.product', $product->id) }}" class="block" x-data="{ imgLoaded: false }">
                        <div class="relative aspect-square bg-surface-container overflow-hidden">
                            ...
                            @endif
                            <div
                                class="absolute top-2 left-2 bg-primary/90 backdrop-blur-sm text-white text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5">
                                <span class="material-symbols-outlined" style="font-size:9px">verified</span> Mitra
                            </div>
                        </div>
                        <div class="p-3">
                            @if($store)
                                <p class="text-[10px] font-bold text-secondary uppercase tracking-wider mb-1 truncate">
                                    {{ $store->name }}</p>
                            @endif
                            <h3 class="text-xs font-bold text-on-surface leading-tight line-clamp-2 mb-1">{{ $product->name }}
                            </h3>
                                ...
                                @if($store && $store->city)
                                    <span class="text-[9px] text-on-surface-variant flex items-center gap-0.5">
                                        <span class="material-symbols-outlined"
                                            style="font-size:10px">location_on</span>{{ $store->city }}
                                    </span>
                                @endif
                            </div>
                        </div>
    ...
    @else
        <div class="py-16 text-center">
            <span class="material-symbols-outlined text-outline-variant block mb-3"
